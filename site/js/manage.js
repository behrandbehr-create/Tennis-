...
(function () {
  ...
  function renderSchedule() {
    const b = $('#manage-body'); const ms = d().matches; const singles = (d().playersPerMatch || 4) <= 2;
    b.innerHTML = '<p style="color:var(--muted)">Tap <b>Score</b> after a match, <b>Edit</b> to change who plays, who brings balls, or the date.</p>' +
      ms.map(m => '<div class="row-edit" data-mid="' + m.id + '"><div style="text-align:center;min-width:64px"><div style="font-family:var(--display);font-size:1.6rem;font-weight:800;line-height:1">' + L.fmtDate(m.date, { day: 'numeric' }) + '</div><div style="font-size:.7rem;color:var(--muted);font-weight:700;text-transform:uppercase">' + L.fmtDate(m.date, { month: 'short' }) + ' · ' + L.dow(m.date) + '</div></div><div><div><span class="pill ' + (m.status === 'played' && !m.sets.length ? 'needs' : m.status) + '">' + (m.status === 'played' && !m.sets.length ? 'Needs a score' : L.statusLabel(m)) + '</span> <b>' + esc(L.names(m.teamA)) + '</b> vs <b>' + esc(L.names(m.teamB)) + '</b></div><div style="font-size:.85rem;color:var(--muted)">' + L.ballHTML() + ' ' + esc(L.pshort(m.balls)) + (m.sets && m.sets.length ? ' · ' + m.sets.map(s => s.a + '-' + s.b + (s.tb ? ' TB' : '')).join(', ') : '') + (m.note ? ' · ' + esc(m.note) : '') + '</div></div><div class="tools"><button class="btn court small" data-score="' + m.id + '">Score</button><button class="btn soft small" data-edit="' + m.id + '">Edit</button><button class="btn danger small" data-delm="' + m.id + '">Delete</button></div></div>').join('') +
      '<div class="card" style="margin-top:16px"><h3>' + (L.isFlex() ? 'Add a match week' : 'Add a match night') + '</h3><div class="form-grid" style="margin-top:10px"><label class="f">' + (L.isFlex() ? 'Week of (Monday)' : 'Date') + '<input type="date" id="nm-date" value="' + esc(suggestDate()) + '"></label>' + (L.isFlex() ? '' : '<label class="f">Start<input type="time" id="nm-start" value="' + esc(d().startTime) + '"></label><label class="f">End<input type="time" id="nm-end" value="' + esc(d().endTime) + '"></label>') + '</div><div class="form-grid" style="margin-top:10px">' + (singles ? '<label class="f">Player 1<select id="nm-a1">' + playerOptions(0, true) + '</select></label><input type="hidden" id="nm-a2" value=""><label class="f">Player 2<select id="nm-b1">' + playerOptions(0, true) + '</select></label><input type="hidden" id="nm-b2" value="">' : '<label class="f">Team A<select id="nm-a1">' + playerOptions(0, true) + '</select></label><label class="f">&nbsp;<select id="nm-a2">' + playerOptions(0, true) + '</select></label><label class="f">Team B<select id="nm-b1">' + playerOptions(0, true) + '</select></label><label class="f">&nbsp;<select id="nm-b2">' + playerOptions(0, true) + '</select></label>') + '</div><div style="margin-top:12px"><button class="btn court" id="nm-add">Add match</button></div></div>';
    ...
    $('#nm-add').onclick = () => { const date = $('#nm-date').value; if (!date) return L.toast('Pick a date'); const ids = ['a1', 'a2', 'b1', 'b2'].map(k => Number($('#nm-' + k).value) || 0); const m = { id: nextId(d().matches), date, startTime: $('#nm-start') ? $('#nm-start').value : d().startTime, endTime: $('#nm-end') ? $('#nm-end').value : d().endTime, teamA: singles ? [ids[0]] : ids.slice(0, 2), teamB: singles ? [ids[2]] : ids.slice(2), balls: ids.find(x => x) || 0, status: 'scheduled', sets: [], note: '' }; if (new Set(ids.filter(x => x)).size !== ids.filter(x => x).length) return L.toast('A player is listed twice'); d().matches.push(m); L.save('Match added'); };
  }
  ...
  function scoreDialog(m) {
    ...
    const draw = () => {
      const singles = L.lineup(m).length <= 2; const labA = singles ? esc(L.names(m.teamA)) : 'Team A', labB = singles ? esc(L.names(m.teamB)) : 'Team B';
      $('#modal-body').innerHTML = '<p style="margin:0 0 10px;color:var(--muted)">' + (L.isFlex() ? 'Week of ' : '') + L.fmtDate(m.date) + (singles ? ' · <b>' + labA + '</b> vs <b>' + labB + '</b>' : ' · Team A: <b>' + esc(L.names(m.teamA)) + '</b> · Team B: <b>' + esc(L.names(m.teamB)) + '</b>') + '</p>' +
        '<div class="status-choice">' + STATUSES.map(s => '<button type="button" data-st="' + s[0] + '" aria-pressed="' + (status === s[0]) + '">' + s[1] + '</button>').join('') + '</div>' +
        (status === 'played' || status === 'unfinished' ? '<div class="set-rows" style="margin-top:14px">' + sets.map((s, i) => '<div class="set-row"><select data-tb="' + i + '" class="chip" style="padding:8px 28px 8px 12px">' + opt('0', 'Set ' + (i + 1), s.tb ? '1' : '0') + opt('1', 'Match tiebreak', s.tb ? '1' : '0') + '</select><input type="number" min="0" max="30" inputmode="numeric" data-a="' + i + '" value="' + s.a + '" aria-label="' + labA + ' games"><span style="text-align:center;font-weight:800">:</span><input type="number" min="0" max="30" inputmode="numeric" data-b="' + i + '" value="' + s.b + '" aria-label="' + labB + ' games"><button type="button" class="btn soft small" data-rm="' + i + '" aria-label="Remove set">×</button></div>').join('') + '</div><div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap">' + (sets.length < 5 ? '<button type="button" class="btn soft small" id="add-set">+ Add set</button>' : '') + '<span style="font-size:.8rem;color:var(--muted);align-self:center">Left number is ' + labA + '.</span></div><div id="score-warn"></div>' : '<p class="warn">' + ({ scheduled: 'Scores will be cleared and the match shows as scheduled.', rescheduled: 'Use "Edit" on the Manage tab to set the new date.', canceled: 'The match will not count toward standings.' }[status] || '') + '</p>') +
    ...
    const warn = () => { const el = $('#score-warn'); if (!el) return; const w = L.validateSets(sets, status); const res = L.matchResult({ sets }); const singles = L.lineup(m).length <= 2; el.innerHTML = (w.length ? '<div class="warn">' + w.map(esc).join('<br>') + '</div>' : (status === 'played' && sets.length ? '<div class="ok">' + (singles ? esc(L.names(res === 1 ? m.teamA : m.teamB)) + ' wins ' : (res === 1 ? 'Team A' : 'Team B') + ' wins ') + sets.map(s => s.a + '-' + s.b).join(', ') + '.</div>' : '')); };
    L.openModal('Match result', ''); draw();
  }

  /* ---------- lineup / date dialog ---------- */
  function lineupDialog(m) {
    const singles = (d().playersPerMatch || 4) <= 2; const flex = L.isFlex();
    L.openModal('Edit match', '<div class="form-grid"><label class="f">' + (flex ? 'Week of (Monday)' : 'Date') + '<input type="date" id="lm-date" value="' + esc(m.date) + '"></label>' + (flex ? '' : '<label class="f">Start<input type="time" id="lm-start" value="' + esc(m.startTime || d().startTime) + '"></label><label class="f">End<input type="time" id="lm-end" value="' + esc(m.endTime || d().endTime) + '"></label>') + '</div>' +
      '<div class="form-grid" style="margin-top:10px"><label class="f">' + (singles ? 'Player 1' : 'Team A') + '<select id="lm-a1">' + playerOptions(m.teamA[0], true) + '</select></label>' + (singles ? '<input type="hidden" id="lm-a2" value="">' : '<label class="f">&nbsp;<select id="lm-a2">' + playerOptions(m.teamA[1], true) + '</select></label>') + '<label class="f">' + (singles ? 'Player 2' : 'Team B') + '<select id="lm-b1">' + playerOptions(m.teamB[0], true) + '</select></label>' + (singles ? '<input type="hidden" id="lm-b2" value="">' : '<label class="f">&nbsp;<select id="lm-b2">' + playerOptions(m.teamB[1], true) + '</select></label>') + '</div>' +
    ...
    $('#lm-save').onclick = () => {
      ...
      m.date = date; if ($('#lm-start')) { m.startTime = $('#lm-start').value; m.endTime = $('#lm-end').value; } m.teamA = singles ? [ids[0]] : ids.slice(0, 2); m.teamB = singles ? [ids[2]] : ids.slice(2); m.balls = Number($('#lm-balls').value) || real[0] || 0; m.status = $('#lm-status').value; m.note = $('#lm-note').value.trim();
      if (m.status !== 'played' && m.status !== 'unfinished') m.sets = [];
      L.closeModal(); L.save('Match updated');
      if (m.status === 'played' && !m.sets.length) setTimeout(() => { L.toast('Now enter the score'); scoreDialog(m); }, 150);
    };
  }
  ...
  function renderLeague() {
    ...
    $$('[data-l]', b).forEach(i => i.onchange = () => { const k = i.dataset.l; x[k] = k === 'rules' ? i.value.split('\n').map(s => s.trim()).filter(Boolean) : k === 'playersPerMatch' ? Number(i.value) : k === 'flex' ? i.value === 'true' : i.value.trim(); L.save(); });
